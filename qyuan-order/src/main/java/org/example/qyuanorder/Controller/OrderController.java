...
import org.example.qyuanorder.Feign.ReadNoteFeign;
...
public class OrderController {
    ...
    @Resource
    private ReadNoteFeign readNoteFeign;
    ...
    public Result<Object> payOrder(@RequestHeader(value = "USER-ID") int user_id,@PathVariable("order_id") int order_id,
                                   @RequestBody JSONObject body){
        try {
            BigDecimal payAccount=body.getBigDecimal("payAccount");
            String payMethor=body.getString("payMethod");
            Order order=orderService.payOrder(user_id,order_id,payAccount,payMethor);
            return Result.ok(order);
        }catch (Exception  e){
            return Result.fail(e.getMessage());
        }
